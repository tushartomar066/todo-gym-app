...
export default function TodoClient({ initialTasks }: { initialTasks: Task[] }) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      const added = await addTask(newTask.trim(), priority)
      setNewTask('')
      // Optimistically prepend and re-apply filter client-side
      setTasks(prev => {
        const next = [added!, ...prev]
        if (filter === 'completed') return prev
        if (filter === 'active') return [added!, ...prev.filter(t => !t.is_completed)]
        return next
      })
    } catch {
  ...
  const displayed = filter === 'all'       ? tasks
                  : filter === 'active'    ? tasks.filter(t => !t.is_completed)
